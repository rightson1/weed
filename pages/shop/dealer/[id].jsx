...
const Index = () => {
    ...
    const { id: dealer } = router.query;
    useEffect(() => {

        axios.get(`${baseUrl}/product?dealer=${dealer}`).then((res) => {
    ...
    useEffect(() => {
        axios.get(`${baseUrl}/product?offer=${true}&dealer=${dealer}`).then((res) => {
            setLoading1(false);
            setOffers(res.data)
        }).catch((err) => {
            console.log(err)
    ...
    const handleSubmit = (e) => {
        ...
        setSearch(true);
        axios.get(`${baseUrl}/product?search=${search}&dealer=${dealer}`).then((res) => {
            setFound(res.data)
            setLoading2(false);
            e.target.reset()
    ...
    return <>
        ...
        <div className=" p-4 flex flex-col gap-5 md:mr-4 mt-[40px] bg-white">
            ...
            <div className="flex  gap-2 w-full justify-between ">
                <h1 className="font-bold text-2xl">{dealer.toUpperCase()}</h1>
